  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Message text + inline timestamp (WhatsApp / Telegram style).
            For short messages the time sits on the SAME line as the text; for
            long messages that wrap, the time flows to the bottom-right. This is
            what stops a 2-character message from becoming 2 lines tall.
            FIX-13: msgTextAI (italic-tinted) for AI variant (PRD §10.2). */}
        <View style={[styles.bodyRow, isRight && styles.bodyRowRight]}>
          <Text
            style={[
              styles.msgText,
          ...
          {/* Inline time — only when NOT clamped. A clamped long message renders
              its time on its own row below (block fallback further down). */}
          {time && !clampNow ? (
            <View style={[styles.timeRow, styles.timeInline]}>
        ...
        {/* Block timestamp fallback — used only while a long message is clamped */}
        {time && clampNow ? (
...
const styles = StyleSheet.create({
  ...
  msgText: {
    ...
    color:      T.textBody,
    fontWeight: '400',
    flexShrink: 1,                                  // wrap inside the inline-time row
  },
  msgTextRight: {
  ...
  // ── Message body row (text + inline timestamp) ────────────────────────────
  // flexWrap lets a short message keep the time on the same line, while a long
  // message wraps the text and pushes the time to the next line (bottom-right).
  bodyRow: {
    flexDirection: 'row',
    flexWrap:      'wrap',
    alignItems:    'flex-end',
  ...
  // Inline modifier — no top margin, small left gap from the text, baseline nudge.
  timeInline: {
    marginTop:    0,
    marginLeft:   8,
    marginBottom: 1,
