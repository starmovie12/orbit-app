 ...
 *
 * Changelog (v4 — PRD §1.4.3 bio font + stale-closure + dead-import pass):
 *   FIX-18  `heroBio` font changed FONT_BODY.regular → FONT_CORMORANT.medium + fontSize 16
 *           PRD §1.4.3: "Profile bio, quotes, decorative text → Cormorant Garamond 500/600 · 18/16px"
 *   FIX-19  `TITLE_COLORS` dead import removed — imported but never referenced anywhere in file
 *   FIX-20  `refreshTestimonials` stale-closure fixed — `state.data.uid` pulled into stable
 *           `targetUid` derived var so callback does NOT recreate on every state change
 *           (was incorrectly listing entire `state` object in useCallback deps)
 *   FIX-21  `heroBio` lineHeight updated 18 → 22 (Cormorant 16px requires ~1.375× for readability)
 *
 * Changelog (v5 — 3 PRD PR-Block fixes: typography weights · border radius · overlay tokens):
 *   FIX-22  GRADIENT_CARD_OVERLAYS moved from inline component const → @/constants/colors.ts
 *           PRD §1.4.2 "Single source of truth for every pixel. Hardcoded hex anywhere = PR block."
 *           §1.4.7 Anti-Rules: "Hardcoded hex in components — Only design tokens — lint rule blocks PR"
 *           Raw rgba() values now imported as `gradientCardOverlays` from colors constants.
 *           See § 1 migration block for exact lines to add to @/constants/colors.ts.
 *   FIX-23  `heroName` typography fixed: FONT_HEADING.semiBold ('700') → FONT_HEADING.extraBold ('800')
 *           PRD §1.4.3: "Headings → Syne | weight 800" — semiBold (Syne_700) is not in the approved set.
 *   FIX-24  `countPillText` Inter weight fixed: FONT_BODY.bold ('700') → FONT_BODY.semiBold ('600')
 *   FIX-25  `qrTitle` Inter weight fixed: FONT_BODY.bold ('700') → FONT_BODY.semiBold ('600')
 *   FIX-26  `sharePrimaryBtnText` Inter weight fixed: FONT_BODY.bold ('700') → FONT_BODY.semiBold ('600')
 *   FIX-27  `submitBtnText` Inter weight fixed: FONT_BODY.bold ('700') → FONT_BODY.semiBold ('600')
 *           PRD §1.4.3 for all FIX-24–27: "Body/UI → Inter | 400 / 500 / 600 only" — 700 is banned.
 *   FIX-28  `cardHero` borderRadius: radii['2xl'] (20px) → radii.lg (16px)
 *           PRD §1.4.4 radius scale: 4 · 8 · 12 · 16 · 18 · 999 · 0 ONLY. 20px does not exist.
 */
...
import {
  ...
  touch,
  // FIX-22: gradientCardOverlays imported from token system — removes inline raw rgba() from component.
  // PRD §1.4.2: "Single source of truth for every pixel. Hardcoded hex anywhere = PR block."
  // See § 1 migration guide below for what must be added to @/constants/colors.ts.
  gradientCardOverlays as GRADIENT_CARD_OVERLAYS,
} from '@/constants/colors';
import {
  FONT_HEADING,
  FONT_BODY,
  FONT_NUMERIC,
  FONT_CORMORANT,          // FIX-18: PRD §1.4.3 — "Profile bio, quotes, decorative text → Cormorant Garamond"
} from '@/constants/typography';
import { TITLES, TITLE_LABELS, type TitleTier } from '@/constants/titles';
// FIX-19: `TITLE_COLORS` removed — was imported but never referenced anywhere in this file
...
// ─────────────────────────────────────────────────────────────────────────────
// § 1 — FIX-22 MIGRATION GUIDE — GRADIENT_CARD_OVERLAYS → @/constants/colors.ts
//
// PRD §1.4.2 + §1.4.7 violation resolved: raw rgba() values removed from this
// component file. They now come from `gradientCardOverlays` imported above.
//
// ── REQUIRED: add the following export to @/constants/colors.ts ──────────────
//
//   /**
...
export default function CrownCardScreen() {
  ...
  // FIX-20: Derive targetUid as a stable primitive so refreshTestimonials does NOT
  // recreate on every unrelated state change (e.g. sharing=true, which was triggering
  // re-creation because `state` (the entire object) was in the dependency array).
  // Stable uid → stable callback → no unnecessary re-renders downstream.
  const targetUid: string | null =
    state.status === 'success' ? state.data.uid : null;
  ...
  // FIX-20: targetUid is a stable string | null — recreates ONLY when uid changes,
  // NOT on every state mutation (sharing, etc.). Was: [state] (entire object).
  }, [targetUid]);
...
const styles = StyleSheet.create({
  ...
  countPillText: {
    // FIX-24: FONT_BODY.bold ('700') → FONT_BODY.semiBold ('600')
    // PRD §1.4.3: "Body/UI → Inter | 400/500/600 only". fontWeight 700 is banned for Inter.
    fontFamily:  FONT_BODY.semiBold,
  ...
  cardHero: {
    ...
    // FIX-28: radii['2xl'] was 20px — not in PRD §1.4.4 scale (4·8·12·16·18·999·0).
    // radii.lg = 16px (modal/sheet/prominent card) — closest valid scale value.
    borderRadius:    radii.lg,            // 16 ✓ (was 20 — banned off-scale value)
    overflow:        'hidden',
  ...
  heroName: {
    // FIX-23: PRD §1.4.3 "Headings → Syne | weight 800" — semiBold (Syne_700) was not in approved set.
    // FONT_HEADING.semiBold loaded Syne_700Bold which is banned. FONT_HEADING.extraBold = Syne_800ExtraBold.
    fontFamily:       FONT_HEADING.extraBold,  // Syne_800ExtraBold — only approved Syne weight
    color:            palette.white,
    fontSize:         FS_HERO_NAME,             // FIX-16: own constant (20) ✓
    fontWeight:       '800',                    // FIX-23: was '700' (Syne only 800 per PRD §1.4.3)
  ...
  heroBio: {
    // FIX-18: PRD §1.4.3 — "Profile bio, quotes, decorative text → Cormorant Garamond 500/600 · 18/16px"
    // Was: FONT_BODY.regular (Inter) — violates §1.4.3 font assignment for bio context.
    fontFamily:  FONT_CORMORANT.medium,  // Cormorant Garamond 500
    color:       GRADIENT_CARD_OVERLAYS.bio,              // FIX-09: was WHITE_82
    fontSize:    16,                     // FIX-18: Cormorant scale: 18/16 ✓ (was 13 — Inter scale)
    fontWeight:  '500',
    // FIX-21: lineHeight updated 18 → 22 (Cormorant 16px serif needs ~1.375× for readability)
    lineHeight:  22,                     // Typography line-height, not layout spacing
    marginTop:   spacing.md,             // 12 ✓
  },
  heroBadgeRow: {
    flexDirection: 'row',
  ...
  statVal: {
    ...
    fontWeight:    '700',                // ✅ APPROVED: PRD §1.4.3 "Numbers → Space Mono | weight 700"
    letterSpacing: -0.2,
  },
  statLbl: {
    fontFamily:    FONT_BODY.semiBold,
  ...
  trustVal: {
    fontFamily:    FONT_NUMERIC.bold,
    fontSize:      FS_TRUST_SCORE,       // 20 — Space Mono scale ✓
    fontWeight:    '700',                // ✅ APPROVED: PRD §1.4.3 "Numbers → Space Mono | weight 700"
    letterSpacing: -0.4,
  },
  trustTrack: {
  ...
  qrTitle: {
    // FIX-25: FONT_BODY.bold ('700') → FONT_BODY.semiBold ('600')
    // PRD §1.4.3: Inter max weight is 600. fontWeight 700 is banned.
    fontFamily:    FONT_BODY.semiBold,
    color:         crown.textPrimary,    // FIX-12: was orbit.textPrimary
    fontSize:      15,
    fontWeight:    '600',
  ...
  sharePrimaryBtnText: {
    // FIX-26: FONT_BODY.bold ('700') → FONT_BODY.semiBold ('600')
    // PRD §1.4.3: Inter 400/500/600 only. fontWeight 700 banned.
    fontFamily:  FONT_BODY.semiBold,
  ...
  submitBtnText: {
    // FIX-27: FONT_BODY.bold ('700') → FONT_BODY.semiBold ('600')
    // PRD §1.4.3: Inter 400/500/600 only. fontWeight 700 banned.
    fontFamily:  FONT_BODY.semiBold,
    color:       palette.white,          // FIX-17: was orbit.white → palette.white
    fontSize:    13,
