 ...
 * Settings — §5.9 spec
 *
 * Top bar: back + "Settings" + search icon.
 * Grouped sections (overline labels).
 * List rows per §4.4, each with a small Lucide icon in a 32×32 rounded surface2 square
 * (monochrome and quiet — NOT colored neon tiles like Telegram).
 */
  ...
  const groups: Group[] = [
    {
      title: 'ACCOUNT',
      rows: [
        { icon: 'user',         label: 'Edit Profile',              hint: 'Name, handle, bio' },
        { icon: 'phone',        label: 'Phone Number',              hint: '+91 ••••• 43210' },
        { icon: 'mail',         label: 'Email',                     hint: 'Not set' },
        { icon: 'key',          label: 'Change PIN / Password' },
      ],
    },
    {
      title: 'NOTIFICATIONS',
      rows: [
        { icon: 'bell',         label: 'Push notifications',        toggle: true, value: notifPush,  onPress: () => setNotifPush(v => !v) },
        { icon: 'volume-2',     label: 'Sounds',                    toggle: true, value: notifSound, onPress: () => setNotifSound(v => !v) },
        { icon: 'moon',         label: 'Do not disturb',            hint: 'Off' },
      ],
    },
    {
      ...
      rows: [
        ...
        { icon: 'shield',       label: 'DPDP Privacy Settings',     hint: 'Data rights' },
        { icon: 'user-x',       label: 'Blocked accounts' },
        { icon: 'eye',          label: 'Active sessions',           hint: '2 devices' },
      ],
    },
    {
      title: 'WALLET',
      rows: [
        { icon: 'credit-card',  label: 'Payment methods',           hint: '1 card · UPI linked' },
    ...
    },
    {
      ...
      rows: [
        ...
        { icon: 'map-pin',      label: 'Region',                    hint: 'India' },
        { icon: 'droplet',      label: 'Appearance',                hint: 'Dark' },
        { icon: 'bar-chart-2',  label: 'Usage analytics',           toggle: true, value: analytics, onPress: () => setAnalytics(v => !v) },
      ],
    },
    {
      title: 'DATA',
                  ...
                  >
                    ...
                    {row.toggle ? (
                      <View
                        style={[
                          styles.switch,
                          { backgroundColor: row.value ? orbit.accent : orbit.surface3 },
                        ]}
                      >
                        <View
                          style={[
                            styles.switchKnob,
                            {
                              transform: [{ translateX: row.value ? 16 : 0 }],
                            },
                          ]}
                        />
                      </View>
